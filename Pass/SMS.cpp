...
static cl::opt<bool> EnableSWP("sms-enable-pipeliner", cl::Hidden, cl::init(true),
                               cl::ZeroOrMore,
                               cl::desc("Enable Software Pipelining"));

/// A command line option to enable SWP at -Os.
static cl::opt<bool> EnableSWPOptSize("sms-enable-pipeliner-opt-size",
                                      cl::desc("Enable SWP at Os."), cl::Hidden,
                                      cl::init(false));

/// A command line argument to limit minimum initial interval for pipelining.
static cl::opt<int> SwpMaxMii("sms-pipeliner-max-mii",
                              cl::desc("Size limit for the MII."),
                              cl::Hidden, cl::init(27));

/// A command line argument to limit the number of stages in the pipeline.
static cl::opt<int>
    SwpMaxStages("sms-pipeliner-max-stages",
...
/// an unrelated Phi.
static cl::opt<bool>
    SwpPruneDeps("sms-pipeliner-prune-deps",
...
/// dependences.
static cl::opt<bool>
    SwpPruneLoopCarried("sms-pipeliner-prune-loop-carried",
                        cl::desc("Prune loop carried order dependences."),
                        cl::Hidden, cl::init(true));

#ifndef NDEBUG
static cl::opt<int> SwpLoopLimit("sms-pipeliner-max", cl::Hidden, cl::init(-1));
#endif

static cl::opt<bool> SwpIgnoreRecMII("sms-pipeliner-ignore-recmii",
                                     cl::ReallyHidden, cl::init(false),
                                     cl::ZeroOrMore, cl::desc("Ignore RecMII"));

static cl::opt<bool> SwpShowResMask("sms-pipeliner-show-mask", cl::Hidden,
                                    cl::init(false));
static cl::opt<bool> SwpDebugResource("sms-pipeliner-dbg-res", cl::Hidden,
                                      cl::init(false));

static cl::opt<bool> EmitTestAnnotations(
    "sms-pipeliner-annotate-for-testing", cl::Hidden, cl::init(false),
...
static cl::opt<bool> ExperimentalCodeGen(
    "sms-pipeliner-experimental-cg", cl::Hidden, cl::init(false),
    cl::desc(
...
namespace llvm
{
  ...
  cl::opt<bool>
      SwpEnableCopyToPhi("sms-pipeliner-enable-copytophi", cl::ReallyHidden,
...
bool SMS::runOnMachineFunction(MachineFunction &mf)
{
...
/// the loop.
bool SMS::scheduleLoop(MachineLoop &L)
{
  bool Changed = false;
...
void SMS::setPragmaPipelineOptions(MachineLoop &L)
{
...
bool SMS::canPipelineLoop(MachineLoop &L)
{
  if (L.getNumBlocks() != 1)
  {
    ORE->emit([&]()
...
void SMS::preprocessPhiNodes(MachineBasicBlock &B)
{
...
bool SMS::swingModuloScheduler(MachineLoop &L)
{
...
void SMS::getAnalysisUsage(AnalysisUsage &AU) const
{
...
bool sortCostVecAsap(const std::pair<int, int> &a,
...
bool sortCostVecAlap(const std::pair<int, int> &a,
                     const std::pair<int, int> &b)
{
  if (a.first != b.first)
    return (a.first < b.first);
...
bool SMSchedule::insert(SUnit *SU, int StartCycle, int EndCycle, int II)
{
  ...
  if (forward)
    std::sort(costVec.begin(), costVec.end(), sortCostVecAsap); // sort the pairs by first element, the cost, in asending order
  else
    std::sort(costVec.begin(), costVec.end(), sortCostVecAlap);
...
static RegisterPass<SMS> Y("sms", "Swing Modolo Scheduling", false, false);